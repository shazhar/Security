/* Updated version of Craig H. Rowland's code by Shahira A. Azhar March 8, 2020
* 
...
void forgeclient ( unsigned int source_addr, unsigned int dest_addr, unsigned short source_port, unsigned short dest_port, char *filename, int ipid, int seq, int win, int minDelay, int maxDelay );
void forgeserver (unsigned int source_addr, unsigned int dest_addr, unsigned short source_port, unsigned short dest_port, char *filename, int ipid, int seq, int win, int ack);
...
void forgeclient ( unsigned int source_addr, unsigned int dest_addr, unsigned short source_port, unsigned short dest_port, char *filename, int ipid, int seq, int win, int minDelay, int maxDelay )
{
	...
	while (( ch=fgetc(input) ) !=EOF )
 	{
		// Send data over random intervals within the specified range
	...
	while(1) /* read packet loop */
	{
		...
		if ( recv_pkt.tcp.syn == 1 )
		{
			if ( recv_pkt.ip.saddr == source_addr  || ntohs(recv_pkt.tcp.dest) == source_port )
			{
				...
				if ( ipid )
					data = recv_pkt.ip.id;
				// TCP Sequence number "decoding"
				else if ( seq )
					data = recv_pkt.tcp.seq;
				// TCP Window size "decoding"
				else if ( win )
					data = recv_pkt.tcp.window;
				// Bounced packet "decoding" seq
				else if ( ack )
					data = recv_pkt.tcp.ack_seq;

				printf ( "INFO: Receiving Data: %c\n", data );
				if ( fprintf ( output,"%c", data) < 0 )
...
int main ( int argc, const char** argv )
{
   ...
   struct poptOption optionsTable[] = 
   {
		...
		{ "win"         , 'w' , POPT_ARG_NONE   , NULL         , 'w' , "TCP Window Encoding"               		    },
		{ "server"      , 'S' , POPT_ARG_NONE   , NULL         , 'S' , "Run as server"              				},
		{ "minDelay"    , 'm' , POPT_ARG_INT    , &minDelay    , 'm' , "Minimum delay (s) between data transfer" 	},
   		{ "maxDelay"    , 'M' , POPT_ARG_INT    , &maxDelay    , 'M' , "Maximum delay (s) between data transfer" 	},
	   	POPT_AUTOHELP
	   	POPT_TABLEEND
   };	
  ...
  if ( getuid() != 0 )
   ...
   if ( !server )
   {
   		...
   		else if ( !( ipid ^ seq ^ win))
   		{
   			printf("ERROR: You must choose to encode one of ipid, seq or win!\n");
   			exit(1);
   		}
   		else if ( !minDelay || !maxDelay )
   		{
   			printf("ERROR: Client will send data at random intervals within a range. You must specify the minimum and maximum delay! ( --minDelay <int> --maxDelay <int> )\n");
   			exit(1);
   		}
   		else if ( minDelay > maxDelay )
   		{
   			printf("ERROR: Your minimum delay must be less than or equal to the maximum delay!\n");
   			exit(1);
   		}
   		else
   		{
   ...
   else
   {
   		...
   		if ( ! ( ipid ^ seq ^ ack ^ win))
   		{
   			printf("ERROR: You must choose exactly one decoding method (ipid, seq, win, or ack)!\n");
   			exit(1);
   		}
	...
	srand( getpid() * time(0) ); 
	if ( server )
    	forgeserver ( source_host, dest_host, source_port, dest_port, filename, ipid, seq, win, ack );
    else
    	forgeclient ( source_host, dest_host, source_port, dest_port, filename, ipid, seq, win, minDelay, maxDelay );
